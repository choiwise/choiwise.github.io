...
### 3. [관련 이미지와 동영상](#이미지)
### 4. [대표 이미지](#대표이미지)
### 5. [대표이미지 기반 작품 묘사](#작품묘사)   
### 6. [구성요소](#구성요소)
### 7.[게임 시스템 디자인](#시스템디자인) 
### 8.[개발 요구사항 & 흐름도](#요구사항)
### 8-2. [시간별 흐름도](#흐름도)
### 8-3. [키보드 이벤트에 대한 흐름도](#키보드)
### 8. [용어 정리](#용어정리)
### 9. [스토리보드](#스토리보드)
### 10. [6주 요구사항](#6요구사항)
### 11. [1주차 작업 결과](#1)
...
- 
-  궁금증을 메인컨셉으로 잡은 이유는 제한된 정보를 제공하여 게임이 플레이 되었을때 왜 플레이어가지"></a>

<img src="./img/대표img.png">

<br><br>

# 5.[컨셉 & 대표이미지 기반 작품묘사] <a name="작품묘사"></a>
...
# 6.[<space in dreams> 구성 요소] <a name="구성요소"></a>
...
## 7.게임 시스템 디자인  <a name="시스템디자인"></a>

# 7.1 게임 오브젝트 분해 (구성 요소 분석)
...
1) 오브젝트 이름: 플레이어
 ...
 |현상태|전이상태|전이조건 |
 |:---:|:---:|:---:|
 |일반상태|느려짐!|운전대 아이템 획득시 5초간 느려짐 효과 발동 |
 |일반상태|시야 가리기!|플레이어가 손전등 아이템 획득시 5초간 화면이 검은 상태로 보이고 그동안 무적상태로 있음 |
 |일반상태|산소부족!|플레이어가 상자 획득시 30초 안에 일정 개수의 산소 획득시 디버프 해제 30초 타임 아웃시 사망 판정 후 리스폰 |
 |일반상태|정신 분열!|마지막 스테이지에서 15초마다 2초동안 앞뒤 양옆 이동 키가 반대로 적용 |

오브젝트 이름: 적(ai) 
 |현상태|전이상태|전이조건 |
 |:---:|:---:|:---:|
 |일반상태|되돌아 가기|플레이어를 리스폰 시킨후에 제자리로 돌아간뒤 다시 플레이어 추격  |
 |일반상태|추격|플레이어를 사각지대에서 놓친 후에 1초 정도 멈춰서 길을 파악한 뒤 다시 플레이어를 추격합니다. |
 |일반상태|광폭화!|플레이어가 산소를 일정% 획득하면 5초간 이동속도가 20% 상승한다. |
...
- 느려짐은 플레이어의 이동속도가 20% 감소하고 시야 가리기는 5초동안 플레이어 시야를 가린다. 시야를 가리는 동안은 무적이다. 
...
23.업적 종류에는 산소부족!인 상태로 체력0으로  죽음. 노데미지 클리어, 리스폰0인 상태로 클리어,~~분 이내 클리어, 맵의 모든 아이템 획득 등 이 있다.  
...
# 8.2 시간별 흐름도 Flow chart  <a name="흐름도"></a>

<img src="./img/시간별flowchart.png" width="700">

# 8.3 키보드 이벤트에 대한 흐름도 <a name="키보드"></a>

<img src="./img/키 입력.png" width="700">

# 8.4 용어 정리 <a name="용어정리"></a>

<img src="./img/용어정리.png" width="600">

#  9. 스토리보드 <a name="스토리보드"></a>

|상황|설명|이미지|
|:----:|:----:|:----:|
|#시네마틱1|#어떤 행성의 발견 우주인(플레이어)는 어떤 행성을 발견한다.|<img src="./img/story1.png" width="300">|
|#시네마틱2|#우주선의 불시착 플레이어는 행성에 불시착 하게된다. 우주선이 부서져 수리를 해야하는 우주인(플레이어)|<img src="./img/story2.png" width="300">|
|#플레이1|행성의 외계인을 피해 산소를 얻으며 부품을모아 우주선을 수리해야 하는 우주인(플레이어) |<img src="./img/story3.png" width="300">|

#  10. 6주 요구사항 <a name="6요구사항"></a>

~~1.플레이어는 1인칭 카메라로 한다. (14)100%~~


~~2.이동은 wasd키를 이용해 이동한다. 시점은 마우스를 돌려 이동 하도록 제작한다.(6)100%~~

 
~~3.스테이지 마다 플레이어 리스폰 지역 start point를 설정 한다.(7)100%~~

 
~~4.적은 랜덤 리스폰을 위해 랜덤 리스폰 point 설정 한다. (8) 100~~

 
~~5.적 이동속도는 플레이어의 70%로 설정한다. (9) 100%~~

 
~~6.플레이어는 적 오브젝트와 닿으면 애니메이션,음악 플레이 후 처음부터 다시 시작된다.(10)100%~~
...
~~8.맵에 산소는 맵당 100~ 180개 사이로 제작. (16)100%~~
...
9.산소 아이템이 전체 개수의 60%이상 획득된 상태이면 랜덤 아이템을 뿌리도록 한다.(11)

 
~~10.스테이지 입장시 가장 오른쪽 위에 맵ui를 볼 수 있는 지도 ui를 배치 한다. (4)~~

 
~~11. 왼쪽 위에 남은 얻어야 할 산소 양이 있다. 산소 아이템 획득시 차감 산소 모두 획득시 스테이지 클리어. (5) 100~~

 
12.플레이어가 랜덤 아이템을 획득하면 바로 효과가 발동 된다.(18)



13.효과는 느려짐!.시야 가리기!,산소 부족! 디버프이다.(19)

- 느려짐은 플레이어의 이동속도가 20% 감소하고 시야 가리기는 5초동안 플레이어 시야를 가린다. 시야를 가리는 동안은 무적이다.

 
14. 플레이어가 산소 부족! 상태면 1초마다 3의 체력이 깎이게 된다. 체력이0이 되면 플레이어는 리스폰된다.(20)

 
~~15.기본 배경음악은 긴장감을 올려줄 호러 음악이다.(28)~~

 
~~16.적이 근처에 올 때 나타날 음악 필요.(29)~~

 
~~17.적이 근처에 접근 시 배경 음악 대신 적 음악 플레이. (15)~~

 
18.마지막 스테이지는 15초마다 2초동안 키입력이 반대로 입력되도록 한다.(17)


# 11. 구현 1주차 작업 결과  <a name="1"></a>
...
플레이시 vertical 과 horizontal 을 이용해 wasd 앞뒤양옆 이동 구현 
...
3.스테이지 마다 플레이어 리스폰 지역 start point를 설정 한다.(7)

 

4.적은 랜덤 리스폰을 위해 랜덤 리스폰 point 설정 한다. (8)

 

5.적 이동속도는 플레이어의 70%로 설정한다. (9)

 

6.플레이어는 적 오브젝트와 닿으면 애니메이션,음악 플레이 후 처음부터 다시 시작된다.(10)
...
8.맵에 산소는 맵당 120~ 180개 사이로 제작. (16)
...
스테이지 5 제작, 스테이지5 맵 산소 배치 100%

스테이지 1,5 플레이어 start point 제작 100%

스테이지 1,5 적 리스폰 point  제작  60%

oncoliderEnter 함수를 이용해 적이 플레이어와 닿으면  애니메이션과 음악 재생후 다시 시작 하도록 함수 구현 0% 

스테이지 산소 왼쪽 상단UI 개수 표시 100% 
...
10.스테이지 입장시 가장 오른쪽 위에 맵ui를 볼 수 있는 지도 ui를 배치 한다. (4)  100%



11. 왼쪽 위에 남은 얻어야 할 산소 양이 있다. 산소 아이템 획득시 차감 산소 모두 획득시 스테이지 클리어. (5) 90% 



4.적은 랜덤 리스폰을 위해 랜덤 리스폰 point 설정 한다. (8)  80% 



15.기본 배경음악은 긴장감을 올려줄 호러 음악이다.(28) 100%



17.적이 근처에 접근 시 배경 음악 대신 적 음악 플레이. (15) 100%
...
땅에서만 움직이도록 코드를 수정 하여 isGround가 true 일때만 움직이게 된다. 
10.스테이지 입장시 가장 오른쪽 위에 맵ui를 볼 수 있는 지도 ui를 배치 한다. (4)  100%

스테이지 입장시 오른쪽 상단 ui지도를 제작하였다. + - 버튼을 통해 크기 조절을 할 수 있도록 제작 완료.



11. 왼쪽 위에 남은 얻어야 할 산소 양이 있다. 산소 아이템 획득시 차감 산소 모두 획득시 스테이지 클리어. (5) 90% 

게임 시작시 왼쪽상단에 산소의 개수가 출력되고 산소가 줄어들때 1씩 감소한다. 산소를 모두 획득하면 스테이지 클리어 
...
적의 리스폰될 구역이4구역있다. 그중 적은  3구역을 받아와 랜덤으로 리스폰 지역을 설정 한다. 이중 게임 시작시 배열중 랜덤으로 

장소를 가지게 되는데 현재 적 enemy가 리스폰되고 넘어지는 현상이 있다.
...
6.플레이어는 적 오브젝트와 닿으면 애니메이션,음악 플레이 후 처음부터 다시 시작된다.(10) 80%

플레이어가 적 오브젝트와 닿으면 처음부터 다시 실행된다. 알파버전은 애니메이션,음악이 적용되어 있지 않습니다.

5.적 이동속도는 플레이어의 70%로 설정한다. (9) 100%

적 이동속도는 맵이 너무 작고 적이 많아 생각보다 빠르면 플레이가 안되므로 약 3~40%정도의 수치를 주었습니다.

13.효과는 느려짐!.시야 가리기!,산소 부족! 디버프이다.(19)  20%

현재 DebuffManager스크립트만 있는 상태입니다.


9.산소 아이템이 전체 개수의 60%이상 획득된 상태이면 랜덤 아이템을 뿌리도록 한다.(11) 0%
...
9.산소 아이템이 전체 개수의 60%이상 획득된 상태이면 랜덤 아이템을 뿌리도록 한다.(11) 40%

12.플레이어가 랜덤 아이템을 획득하면 바로 효과가 발동 된다.(18)

13.효과는 느려짐!.시야 가리기!,산소 부족! 디버프이다.(19)

느려짐은 플레이어의 이동속도가 20% 감소하고 시야 가리기는 5초동안 플레이어 시야를 가린다. 시야를 가리는 동안은 무적이다. 70% 
플레이어가 산소 부족! 상태면 1초마다 3의 체력이 깎이게 된다. 체력이0이 되면 플레이어는 리스폰된다.(20) 100% 
18.마지막 스테이지는 15초마다 2초동안 키입력이 반대로 입력되도록 한다.(17)
...
현재 코드는 존재하지만 산소를 60%이상 획득해도 아이템을 뿌리지 않아 아이템 생성 방식 변경  필요 
...
느려짐은 플레이어의 이동속도가 20% 감소하고 시야 가리기는 5초동안 플레이어 시야를 가린다. 시야를 가리는 동안은 무적이다. 70% 현재 느리짐은 기능이 구현이 완료 되었고 시야가리기는 코드는 완성되었지만 시야 가리는 유니티 설정 구현이 되지 않아 디버그만 확인 가능  
플레이어가 산소 부족! 상태면 1초마다 3의 체력이 깎이게 된다. 체력이0이 되면 플레이어는 리스폰된다.(20) 100% 
18.마지막 스테이지는 15초마다 2초동안 키입력이 반대로 입력되도록 한다.(17) 
